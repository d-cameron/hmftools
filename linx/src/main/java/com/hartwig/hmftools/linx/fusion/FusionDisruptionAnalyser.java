...
public class FusionDisruptionAnalyser {
    ...
    public FusionDisruptionAnalyser(
        ...
        if (config.CmdLineConfig.hasValue(RNA_FUSIONS_FILE)) {
            if (mConfig.Threads > 1 && mConfig.hasMultipleSamples()) {
                ...
                mRnaFusionMapper = new RnaFusionMapper(
                        mOutputDir, config.CmdLineConfig, mGeneDataCache, mFusionFinder, mUniqueFusions,
                        mInvalidFusions);
            }
        }
        ...
        if (mRunFusions) {
            LNX_LOGGER.debug("fusion config: requirePhaseMatch({}) allowExonSkipping({}) requireUpstreamBiotypes({})",
                    mFusionConfig.RequirePhaseMatch, mFusionConfig.AllowExonSkipping,
                    mFusionConfig.RequireUpstreamBiotypes);
    ...
    public boolean hasRnaSampleData() {
        return mRnaFusionMapper != null;
    }

    public final List<GeneFusion> getFusions() {
        return mFusions;
    }

    public final List<GeneFusion> getUniqueFusions() {
        return mUniqueFusions;
    }

    public final Map<GeneFusion, String> getInvalidFusions() {
        return mInvalidFusions;
    }

    public final FusionFinder getFusionFinder() {
        return mFusionFinder;
    }

    public final DisruptionFinder getDisruptionFinder() {
        return mDisruptionFinder;
    }

    public final SpecialFusions getSpecialFusions() {
        return mSpecialFusions;
    }
    ...
    public void annotateTranscripts(final List<SvVarData> svList, boolean purgeInvalidTranscripts) {
        ...
        for (final SvVarData var : svList) {
            // now that transcripts have been marked as disruptive it is safe to purge any
            // which cannot make viable fusions
                ...
                for (int be = SE_START; be <= SE_END; ++be) {
                    ...
                    for (BreakendGeneData gene : genesList) {
                        ...
                        while (transIndex < gene.transcripts().size()) {
                            ...
                            // only retain transcript which are potential fusion candidates (with exception
                            // for canonical)
                            if (!transcript.isDisruptive() && !validFusionTranscript(transcript)
    ...
    public void run(
        ...
        if (mRunFusions) {
            ...
            mUniqueFusions.stream().filter(x -> x.knownType() != NONE)
                    .forEach(x -> mFusionFinder.setFusionProteinFeatures(x));
        }
            ...
            if (LNX_LOGGER.isDebugEnabled()) {
                for (final GeneFusion fusion : mUniqueFusions) {
                    if (fusion.knownType() != KnownFusionType.NONE) {
                        LNX_LOGGER.debug(
                                "fusion({}:{}) reportable({}) knownType({}) cluster({} sv={} chain={}) SVs({} & {})",
                                fusion.id(), fusion.name(), fusion.reportable(),
                                fusion.knownTypeStr(), fusion.getAnnotations().clusterId(),
                                fusion.getAnnotations().clusterCount(),
                                fusion.getAnnotations().chainInfo() != null
                                        ? fusion.getAnnotations().chainInfo().chainId()
                                        : -1,
        ...
        for (final SvVarData var : svList) {
            ...
            if (var.getCluster().getSvCount() > 1 && var.getCluster().findChain(var) != null) {
                boolean igCandidate = !var.isSglBreakend()
                        && (mFusionFinder.getKnownFusionCache().withinIgRegion(var.chromosome(true),
                                var.position(true)) != mFusionFinder.getKnownFusionCache()
                                        .withinIgRegion(var.chromosome(false), var.position(false)));
            ...
            if (mNeoEpitopeWriter != null) {
                double svCopyNumber = var.isSglBreakend() ? var.copyNumber(true)
                        : (var.copyNumber(true) + var.copyNumber(false)) * 0.5;
    ...
    private void findChainedFusions(final List<SvCluster> clusters) {
        ...
        for (final SvCluster cluster : clusters) {
            ...
            // now all fusions have been gathered from this chain, set the reportable one
            // (if any)
    ...
    private void findChainedFusions(
            final SvCluster cluster, final SvChain chain, final List<GeneFusion> chainFusions,
            final List<ValidTraversalData> validPairs) {
        // look for fusions formed by breakends connected in a chain

        // given a chain sAe - sBe - sCe - sDe, where As and De are the open ends of the
        // chain, the following fusions need to be tested:
        // a) each SV in isolation, ie as a single SV
        // b) the left-most / lower breakend of each SV with the right-most / upper
        // breakend of SVs higher up in the chain

        // whenever a linked pair is traversed by a fusion, it cannot touch or traverse
        // genic regions without disrupting the fusion
        ...
        for (int lpIndex1 = 0; lpIndex1 <= linkedPairs.size(); ++lpIndex1) {
            ...
            // the lower link takes the other breakend of the current linked pair's 'first'
            // SV
            // and in order for it to also test the last SV in isolation, also takes the
            // last pair's second (upper) breakend
            ...
            for (int lpIndex2 = lpIndex1; lpIndex2 <= linkedPairs.size(); ++lpIndex2) {
                ...
                // which for the first index will just be the curent SV's 2 breakends - ie
                // testing a single SV in a chain
                // and beyond all the links, it must also test fusions with the chain's upper
                // open breakend
                ...
                // if a new linked pair has been traversed to reach this upper breakend, record
                // its length
                // and test whether it traverses any genic region, and if so invalidate the
                // fusion
                ...
                if (genesListUpper.isEmpty()) {
                    // skip past this link and breakend to the next one, keeping the possibility of
                    // a fusion with the lower breakend open
                    continue;
                }
                ...
                if (mNeoEpitopeWriter != null) {
                    ...
                    double svCopyNumber = upperBreakend != null && lowerBreakend != null
                            ? (lowerBreakend.copyNumber() + upperBreakend.copyNumber()) * 0.5
                            : lowerSV.copyNumber(true);
                ...
                for (GeneFusion fusion : fusions) {
                    // if the fusion from the upstream gene is on the positive strand, then it will
                    // have a fusion direction of +1
                    // whenever it goes through a subsequent linked pair by joining to the first
                    // (lower) breakend in the pair
                    ...
                    final List<BreakendGeneData> downstreamGenes = lowerBreakendPos == fusion.downstreamTrans().gene()
                            .position()
                                    ? genesListLower
                                    : genesListUpper;

                    boolean nonDisruptiveChain = !traversedPairs.isEmpty() && !downstreamGenes.isEmpty()
                            ? mDisruptionFinder.isNonDisruptiveChainedPair(fusion.upstreamTrans(), downstreamGenes)
                            : false;
                    ...
                    // get the downstream gene ID to exclude when checking the last traversed pair
                    String downstreamGeneId = fusion.downstreamTrans().gene().geneId();
                    int lastPairIndex = traversedPairs.size() - 1;

                    for (int i = 0; i < traversedPairs.size(); ++i) {
                        LinkedPair pair = traversedPairs.get(i);
                        ...
                        // if going lower to upper, if the orientation of the first breakend in the pair
                        // is opposite to the strand of
                        // the upstream gene, then the fusion direction for that pair is the same as a
                        // the upstream gene
                        // otherwise it needs to be switched
                        int fusionDirection = 0;

                        if (fusionLowerToUpper) {
                            fusionDirection = pair.firstBreakend().orientation() != upGeneStrand ? upGeneStrand
                                    : -upGeneStrand;
                        } else {
                            fusionDirection = pair.secondBreakend().orientation() != upGeneStrand ? upGeneStrand
                                    : -upGeneStrand;
                        }

                        // any invalid traversal causes this fusion to be entirely skipped from further
                        // analysis
                        // for the last pair, exclude the downstream gene from the traversal check since
                        // crossing its splice acceptor is expected
                        boolean isLastPair = (i == lastPairIndex);
                        String excludeGeneId = isLastPair ? downstreamGeneId : null;
                    ...
                    // check that the chain does not disrupt (ie terminate) the rest of the gene,
                    // checking up then downstream in turn
                    for (int fs = FS_UP; fs <= FS_DOWN; ++fs) {
                        BreakendTransData transcript = fs == FS_UP ? fusion.upstreamTrans() : fusion.downstreamTrans();
                        BreakendGeneData gene = fs == FS_UP ? fusion.upstreamTrans().gene()
                                : fusion.downstreamTrans().gene();

                        boolean isLowerBreakend = lowerBreakendPos == gene.position();

                        boolean isChainEnd = (isLowerBreakend && lpIndex1 == 0)
                                || (!isLowerBreakend && lpIndex2 == linkedPairs.size());
                        ...
                        if (isChainEnd) {
                            ...
                            if (breakend == null) {
                                ...
                                // looks from this link outwards past the end of the transcript for any
                                // invalidation of the transcript
                                ...
                                transTerminated[fs] = checkTranscriptDisruptionInfo(breakend, transcript, chain,
                                        linkIndex, logSkip);
                            }
                        }
                    }
    ...
    private boolean hasValidTraversal(
            final List<ValidTraversalData> validPairs, final LinkedPair pair, int fusionDirection,
            boolean isPrecodingUpstream,
            final String excludeGeneId) {
        // when excluding a gene (for the last pair leading to the downstream gene),
        // skip the cache since
        // the validity depends on the specific gene being excluded, which varies per
        // fusion
        if (excludeGeneId != null) {
            return !mDisruptionFinder.pairTraversesGene(pair, fusionDirection, isPrecodingUpstream, excludeGeneId);
        }
    ...
    private boolean hasIdenticalFusion(final GeneFusion newFusion, final List<GeneFusion> fusions) {
        for (GeneFusion fusion : fusions) {
            ...
            if (newFusion.downstreamTrans().hasNegativePrevSpliceAcceptorDistance() != fusion.downstreamTrans()
                    .hasNegativePrevSpliceAcceptorDistance())
                continue;
    ...
    private List<BreakendGeneData> getBreakendGeneList(final SvVarData var, boolean isStart) {
        if (var.isSglBreakend() && !isStart) {
            ...
            if (var.getSglMappings().stream()
                    .anyMatch(x -> mFusionFinder.getKnownFusionCache().withinIgRegion(x.Chromosome, x.Position)))
                return genesList;

            // otherwise check known pairs (including IG pair 3' genes)
            return genesList.stream()
                    .filter(x -> mFusionFinder.getKnownFusionCache().isSingleBreakendCandidate(x.geneName(),
                            x.isUpstream()))
                    .collect(Collectors.toList());
        }
    ...
    private boolean checkTranscriptDisruptionInfo(
        ...
        // starting with this breakend and working onwards from it in the chain, check
        // for any disruptions to the transcript
        ...
        while (linkIndex >= 0 && linkIndex <= chain.getLinkedPairs().size() - 1) {
            ...
            // the chain does not terminate the transcript if the next breakend is now past
            // the end of the transcript or
            ...
            if (nextBreakend.orientation() == 1) {
                ...
                if (nextBreakend.position() < transcript.transStart())
                    break;
                else if (!isUpstream && transcript.codingStart() > 0
                        && nextBreakend.position() < transcript.codingStart())
                    break;
            }

            // continue on if the next SV (ie both its breakends) is non-disruptive in this
            // transcript
    ...
    private List<GeneFusion> extractUniqueFusions() {
        ...
        // find top priority fusion amongst these fusions by SV-pair, setting
        // reportability at the same time
    ...
    private List<BreakendTransData> buildTranscriptList(final List<SvVarData> svList, final List<GeneFusion> fusions) {
        // add all canonical or otherwise reportable transcript and then add any
        // additional transcripts from the fusions
            ...
            for (int be = SE_START; be <= SE_END; ++be) {
                ...
                for (BreakendGeneData geneAnnotation : var.getGenesList(isStart(be))) {
                    for (BreakendTransData transcript : geneAnnotation.transcripts()) {
                        ...
                        else if (mDisruptionFinder.matchesDisruptionTranscript(geneAnnotation.geneId(),
        ...
        for (final GeneFusion fusion : fusionList) {
            if (fusion.reportable() || mFusionConfig.WriteAllVisFusions) {
                ...
                visFusions.add(new VisFusion(
                        ...
                        transDown.geneName(), transDown.transName(), transDown.gene().chromosome(),
                        transDown.gene().position(),
