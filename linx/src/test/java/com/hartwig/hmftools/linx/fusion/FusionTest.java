...
public class FusionTest {
        ...
        public void testReportableFusionComparison() {
                ...
                int codingStart = 300;
                int codingEnd = 998;
                TranscriptData transData = createTransExons(geneId1, transId++, POS_STRAND, exonStarts, 99, codingStart,
                                codingEnd, true, "");
                ...
                codingStart = 10100;
                codingEnd = 10998;
                transData = createTransExons(geneId1, transId++, POS_STRAND, exonStarts, 99, codingStart, codingEnd,
                                true, "");
                transDataList.add(transData);
                ...
                upGenes.addAll(findGeneAnnotationsBySv(geneTransCache, 0, true, CHR_1, 250, ORIENT_FWD,
                                PRE_GENE_PROMOTOR_DISTANCE));
                upGenes.addAll(findGeneAnnotationsBySv(geneTransCache, 1, true, CHR_1, 450, ORIENT_FWD,
                                PRE_GENE_PROMOTOR_DISTANCE));
                upGenes.addAll(findGeneAnnotationsBySv(geneTransCache, 2, true, CHR_1, 650, ORIENT_FWD,
                                PRE_GENE_PROMOTOR_DISTANCE));
                upGenes.addAll(findGeneAnnotationsBySv(geneTransCache, 3, true, CHR_1, 850, ORIENT_FWD,
                                PRE_GENE_PROMOTOR_DISTANCE));
                ...
                downGenes.addAll(findGeneAnnotationsBySv(geneTransCache, 0, false, CHR_1, 10250, ORIENT_REV,
                                PRE_GENE_PROMOTOR_DISTANCE));
                downGenes.addAll(findGeneAnnotationsBySv(geneTransCache, 1, false, CHR_1, 10450, ORIENT_REV,
                                PRE_GENE_PROMOTOR_DISTANCE));
                downGenes.addAll(findGeneAnnotationsBySv(geneTransCache, 2, false, CHR_1, 10650, ORIENT_REV,
                                PRE_GENE_PROMOTOR_DISTANCE));
                downGenes.addAll(findGeneAnnotationsBySv(geneTransCache, 3, false, CHR_1, 10850, ORIENT_REV,
                                PRE_GENE_PROMOTOR_DISTANCE));
                ...
                // the selected fusion is the longest for coding bases and without any exon
                // skipping
        ...
        public void testFusionPrioritisation() {
                // create a set of valid fusions and successively invalidate the top one to test
        ...
        public void testChainedFusions() {
                ...
                TranscriptData transData = new TranscriptData(transId1, transName1, geneId1, isCanonical, POS_STRAND,
                                transStart, transEnd,
                ...
                transData = new TranscriptData(transId2, transName2, geneId2, isCanonical, POS_STRAND, transStart,
                                transEnd,
                                codingStart, codingEnd, BIOTYPE_PROTEIN_CODING, null);
                ...
                // test 1: create a chain of DELs with a single-SV fusion which link between
                // exon 2-3 of upstream to 2-3 of downstream
                ...
                tester.FusionAnalyser.run(tester.SampleId, tester.AllVariants, tester.getClusters(),
                                tester.Analyser.getState().getChrBreakendMap());
                ...
                // test 2: this time a chain from the first to the last variant with the middle
                // 2 going out to non-disruptive locations
                ...
                // test 4: invalid fusion, with a TI beyond the fusion ending in an exon
                // upstream and skipping an exon downstream
                tester.clearClustersAndSVs();
                ...
                fusion = tester.FusionAnalyser.getFusions().stream().filter(x -> x.reportable()).findFirst()
                                .orElse(null);
                ...
                // test 5: test a chained fusion which has invalid traversal since it goes
                // through another gene's splice acceptor
                transDataList = Lists.newArrayList();
                ...
                // fuses the genes with a DEL but which goes through a few remote TIs in between
                // including one which goes through gene 3
                ...
                assertEquals(1, tester.FusionAnalyser.getFusions().stream().filter(x -> x.validChainTraversal())
                                .count());

                fusion = tester.FusionAnalyser.getFusions().stream().filter(x -> x.validChainTraversal()).findFirst()
                                .orElse(null);
                ...
                fusion = tester.FusionAnalyser.getFusions().stream()
                                .filter(x -> x.upstreamTrans().gene().id() == varId3
                                                && x.downstreamTrans().gene().id() == varId7)
        ...
        @Test
        public void testChainedFusionDownstreamGeneTraversal() {
                // Test that pairTraversesGene respects the excludeGeneId parameter.
                // This tests the fix for the ABL1-ASS1 bug where crossing the downstream
                // gene's splice acceptor was incorrectly flagged as invalid traversal.
                //
                // The bug: When checking if a linked pair has valid traversal, the code was
                // checking ALL genes including the downstream fusion gene. But crossing the
                // downstream gene's splice acceptor is expected (that's where the fusion ends).

                LinxTester tester = new LinxTester();
                EnsemblDataCache geneTransCache = createGeneDataCache();
                tester.initialiseFusions(geneTransCache);

                // Create a gene on CHR_1 with a splice acceptor in the middle
                String geneName1 = "GENE1";
                String geneId1 = "ENSG0001";

                List<GeneData> geneList = Lists.newArrayList();
                geneList.add(createEnsemblGeneData(geneId1, geneName1, CHR_1, 1, 1000, 3000));
                addGeneData(geneTransCache, CHR_1, geneList);

                List<TranscriptData> transDataList = Lists.newArrayList();
                int transId1 = 1;
                TranscriptData transData = new TranscriptData(transId1, "ENST0001", geneId1, true, POS_STRAND,
                                1000, 3000, 1050, 2900, BIOTYPE_PROTEIN_CODING, null);

                List<ExonData> exons = Lists.newArrayList();
                exons.add(new ExonData(transId1, 1000, 1200, 1, -1, 1));
                exons.add(new ExonData(transId1, 2000, 2200, 2, 1, 2)); // splice acceptor at 2000
                exons.add(new ExonData(transId1, 2800, 3000, 3, 2, -1));
                transData.setExons(exons);
                transDataList.add(transData);
                addTransExonData(geneTransCache, geneId1, transDataList);

                // Create a simple DEL that will generate a linked pair crossing the splice
                // acceptor
                // DEL from 1500 to 2500 creates a linked pair conceptually crossing exon2 at
                // 2000
                SvVarData var1 = createBnd(0, CHR_1, 1500, 1, CHR_2, 1000, -1);
                SvVarData var2 = createBnd(1, CHR_2, 2000, 1, CHR_1, 2500, -1);
                ...
                // Create a linked pair that spans 1500-2500 on CHR_1, crossing exon2's splice
                // acceptor at 2000
                LinkedPair pair = LinkedPair.from(var1.getBreakend(true), var2.getBreakend(false));

                // Test 1: Without exclusion, the pair should be flagged as traversing
                int fusionDirection = POS_STRAND; // same as gene strand
                boolean isPrecodingUpstream = false;
                boolean traversesWithoutExclusion = tester.FusionAnalyser.getDisruptionFinder()
                                .pairTraversesGene(pair, fusionDirection, isPrecodingUpstream, null);
                assertTrue("Pair crossing gene's splice acceptor should be flagged as traversing",
                                traversesWithoutExclusion);

                // Test 2: With the gene excluded, the pair should NOT be flagged as traversing
                boolean traversesWithExclusion = tester.FusionAnalyser.getDisruptionFinder()
                                .pairTraversesGene(pair, fusionDirection, isPrecodingUpstream, geneId1);
                assertFalse("Pair crossing excluded gene's splice acceptor should NOT be flagged as traversing",
                                traversesWithExclusion);
        }
        ...
        public void testChainedFusionDisuptions() {
                ...
                TranscriptData transData = new TranscriptData(transId1, transName1, GENE_ID_1, isCanonical, POS_STRAND,
                                transStart, transEnd,
                                codingStart, codingEnd, BIOTYPE_PROTEIN_CODING, null);
                ...
                // test 2: a chained fusion with non-disruptive SVs at both ends, means the
                // fusion is not chain terminated
                tester.clearClustersAndSVs();
                ...
                GeneFusion fusion = tester.FusionAnalyser.getUniqueFusions().stream().filter(x -> x.reportable())
                                .findFirst().orElse(null);
                assertTrue(fusion != null);
                assertEquals(var3.id(), fusion.upstreamTrans().gene().id());
                assertEquals(var3.id(), fusion.downstreamTrans().gene().id());
        ...
        private static boolean validateFusionAnnotations(final GeneFusion fusion, boolean validEnds,
